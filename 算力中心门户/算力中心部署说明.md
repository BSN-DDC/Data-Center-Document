...
### 2.2. 工作路径
此文档工作路径设置为 **/bsn**，如果工作路径设置非 **/bsn** 请修改相关docker-compose.yml启动文件
...
- [jar包下载地址](https://data-center.s3.cn-northwest-1.amazonaws.com.cn/index.html#v2.1.0/deployPackageAndConfiguration/packages/)：git clone  
...
- 数据库初始化脚本：[mysql-schema.sql](https://data-center.s3.cn-northwest-1.amazonaws.com.cn/index.html#v2.1.0/deployPackageAndConfiguration/database/mysql/)
...
services:
  mysql:
    ...
    ports:
      - 3306:3306
    volumes:
      - /bsn/mysql/data:/var/lib/mysql
      - /bsn/mysql/conf/my.cnf:/etc/mysql/my.cnf
      - /bsn/mysql/script:/mysql-script
    command:
...
- 应用服务端配置文件压缩包下载：[应用服务配置](https://data-center.s3.cn-northwest-1.amazonaws.com.cn/index.html#v2.1.0/deployPackageAndConfiguration/config-files/nacos-app/)
...
- redis6应用启动配置文件：[redis.conf](https://github.com/BSN-DDC/data-center/blob/main/deployPackageAndConfiguration/config-files/dockerAndShell/Base/redis/conf/redis.conf)
...
services:
  ddc-chainsrv-<ChainName>-service:
    ...
    #   - 32020:32020
    volumes:
      - /bsn/ddc-chainsrv-<ChainName>/pkg/ddc-chainsrv-<ChainName>.jar:/bsn/chainService/ROOT.jar
      - /bsn/ddc-chainsrv-<ChainName>/ddclogs/:/bsn/ddclogs
      - /bsn/ddc-chainsrv-<ChainName>/script/:/bsn/script
      - /etc/hosts:/etc/hosts
...
- 后端相关微服务单个服务独立启动文件：[DDC-Srv](https://github.com/BSN-DDC/data-center/tree/main/deployPackageAndConfiguration/config-files/dockerAndShell/DDC-Srv)
...
services:
  ddc-<service_name>-service:
    ...
    #   - 38021:38021
    volumes:
      - /bsn/ddc-<service_name>/pkg/ddc-<service_name>.jar:/bsn/ddc-<service_name>/ROOT.jar
      - /bsn/ddc-<service_name>/logs/:/bsn/ddclogs/
      - /bsn/ddc-<service_name>/script/:/bsn/script
      - /etc/hosts:/etc/hosts
      - /etc/localtime:/etc/localtime
    environment:
      # 指定时区
...
- 后端微服务一键启动文件： [一键启动文件地址](https://github.com/BSN-DDC/data-center/tree/v2.1.0/deployPackageAndConfiguration/config-files/dockerAndShell/DDC-Srv-QuickStart)
- 如果之前用单个服务启动的方式需要停止并删除容器后方可使用一键启动
- 将 **docker-compose.yml** 放至 **/bsn/DDC-Srv-QuickStart** 目录下
- 将 jar 包放至 **/bsn/DDC-Srv-QuickStart/pkg** 目录下
- 将 **start.sh** 放至 **/bsn/DDC-Srv-QuickStart/script** 目录下
...
mkdir -p /bsn/ddcsrv/{pkg,logs,script}
#docker-compose.yaml实例文件
...
services:
  ddc-oai-service:
    ...
    volumes:
      ...
      - /bsn/ddc-oai/ddclogs/:/bsn/ddclogs/
